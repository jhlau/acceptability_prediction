...
/*
void printSynapse(synapse *syn, int n, std::string str) {
    int i=0;
    printf("\n%s weights:\n", str.c_str());
    for (i=0; i<n; i++) {
        printf("syn[%d].weight  = %f\n", i, syn[i].weight);
    }
}

void printNeuron(neuron *neu, int n, std::string str) {
    int i=0;
    printf("\n%s ac:\n", str.c_str());
    for (i=0; i<n; i++) {
        printf("neu[%d].ac = %f\n", i, neu[i].ac);
...
void printBpttHidden(neuron *bh, int n, int x, std::string str) {
    printf("\n%s ac/er:", str.c_str());
    int i=0;
    for (i=0; i<x; i++) {
        std::stringstream sstm;
        sstm << "bptt[" << i << "]";
        printNeuron(bh, n, sstm.str());
    }
}

void printBpttHistory(int *bh, int n, std::string str) {
    printf("\n%s:", str.c_str());
    int i=0;
    for (i=0; i<n; i++) {
        printf("bptt_history[%d] = %d\n", i, bh[i]);
    }
}

void CRnnLM::printNetwork() {
    printf("layer0_size = %d\n", layer0_size);
    printf("layer1_size = %d\n", layer1_size);
    printf("layerc_size = %d\n", layerc_size);
    printf("layer2_size = %d\n", layer2_size);


    printNeuron(neu0, layer0_size, "neu0 (input)");
    printNeuron(neu1, layer1_size, "neu1 (hidden)");
    printNeuron(neuc, layerc_size, "neuc (class)");
    printNeuron(neu2, layer2_size, "neu2 (output)");
    printSynapse(syn0, layer0_size*layer1_size, "syn0 (input x hidden)");
    if (layerc_size != 0) {
        printSynapse(syn1, layer1_size*layerc_size, "syn1 (hidden x class)");
        printSynapse(sync, layerc_size*layer2_size, "sync (class x output)");
    } else {
        printSynapse(syn1, layer1_size*layer2_size, "syn1 (hidden x class)");
    }
}

void CRnnLM::printBptt() {
    printBpttHidden(bptt_hidden, layer1_size, (bptt+bptt_block+1), "bptt hidden");
    printBpttHistory(bptt_history, bptt+bptt_block+10, "bptt history");
}
*/

int getFileSize(char filename[]) // path to file
{
...
void CRnnLM::setWordClassFile(char *str)
{
    strcpy(wordclass_file, str);
    print_word_class=1;
}
...
void CRnnLM::saveContext(struct neuron *neu1, struct neuron *neu1b)		//useful for n-best list processing
{
    int a;
...
void CRnnLM::restoreContext(struct neuron *neu1, struct neuron *neu1b)
{
    int a;
...
void CRnnLM::saveContext2(struct neuron *neu1, struct neuron *neu1b2)
{
    int a;
...
void CRnnLM::restoreContext2(struct neuron *neu1, struct neuron *neu1b2)
{
    int a;
...
void CRnnLM::initNeurons(struct neuron **neu0_p, struct neuron **neu1_p, struct neuron **neuc_p, struct neuron **neu2_p, int **bptt_history_p, neuron **bptt_hidden_p, struct synapse **bptt_syn0_p) {
    (*neu0_p)=(struct neuron *)calloc(layer0_size, sizeof(struct neuron));
    (*neu1_p)=(struct neuron *)calloc(layer1_size, sizeof(struct neuron));
    (*neuc_p)=(struct neuron *)calloc(layerc_size, sizeof(struct neuron));
    (*neu2_p)=(struct neuron *)calloc(layer2_size, sizeof(struct neuron));
    struct neuron *neu0 = (*neu0_p);
    struct neuron *neu1 = (*neu1_p);
    struct neuron *neuc = (*neuc_p);
    struct neuron *neu2 = (*neu2_p);
    ...
    if (bptt>0) {
        (*bptt_history_p)=(int *)calloc((bptt+bptt_block+10), sizeof(int));
        (*bptt_hidden_p)=(neuron *)calloc((bptt+bptt_block+1)*layer1_size, sizeof(neuron));
        (*bptt_syn0_p)=(struct synapse *)calloc(layer0_size*layer1_size, sizeof(struct synapse));
        int *bptt_history = (*bptt_history_p);
        neuron *bptt_hidden = (*bptt_hidden_p);
        struct synapse *bptt_syn0 = (*bptt_syn0_p);
...
void CRnnLM::initBackupNeurons(struct neuron **neu0b_p, struct neuron **neu1b_p, struct neuron **neucb_p, struct neuron **neu2b_p, struct neuron **neu1b2_p) {
    (*neu0b_p)=(struct neuron *)calloc(layer0_size, sizeof(struct neuron));
    (*neu1b_p)=(struct neuron *)calloc(layer1_size, sizeof(struct neuron));
    (*neucb_p)=(struct neuron *)calloc(layerc_size, sizeof(struct neuron));
    (*neu1b2_p)=(struct neuron *)calloc(layer1_size, sizeof(struct neuron));
    (*neu2b_p)=(struct neuron *)calloc(layer2_size, sizeof(struct neuron));
}
...
void CRnnLM::initNet()
{
    ...
    //output word class to file
    if (print_word_class) {
        fo=fopen(wordclass_file, "wb");
        if (fo==NULL) {
...
void CRnnLM::restoreNet()    //will read whole network structure
{
    ...
    //
    if (syn0==NULL) initNet();		//memory allocation here
    //
...
void CRnnLM::netFlush(struct neuron *neu0, struct neuron *neu1, struct neuron *neuc, struct neuron *neu2)   //cleans all activations and error vectors
{
    int a;
...
void CRnnLM::netReset(struct neuron *neu0, struct neuron *neu1, int *bptt_history, neuron *bptt_hidden, int history[])   //cleans hidden layer activation + bptt history
{
    int a, b;
    ...
    copyHiddenLayerToInput(neu0, neu1);
...
void CRnnLM::computeNet(int last_word, int word, neuron *neu0, neuron *neu1, neuron *neuc, neuron *neu2, int history[])
{
    int a, b, c;
    real val;
...
void CRnnLM::learnNet(int last_word, int word, struct neuron *neu0, struct neuron *neu1, struct neuron *neuc, struct neuron *neu2, int *bptt_history, neuron *bptt_hidden, struct synapse *bptt_syn0, int history[], int counter)
{
    int a, b, c, t, step;
    real beta2, beta3;
    ...
    //TMP: MOTHER OF GOD LOOP, ACCOUNTS FOR 11% OF COMPUTATION
	for (b=vocab_size; b<layer2_size; b++) {
	    if ((counter%10)==0) {	//regularization is done every 10. step
	...
	if (((counter%bptt_block)==0) || (independent && (word==0))) {
        ...
        now = clock();
        // TMP: MOTHER OF GOD MATRIX MULTIPLICATION, ACCOUNTS FOR 24% OF COMPUTATION
		matrixXvector(neu0, neu1, syn0, layer0_size, 0, layer1_size, layer0_size-layer1_size, layer0_size, 1);		//propagates errors 1->0
        // TMP: END MATRIX MULT
        timeA += ((double)(clock()-now)/1000000.0);

        now = clock();
        // TMP: MOTHER OF GOD LOOP, ACCOUNTS FOR 40% OF COMPUTATION
        ...
        // TMP: END LOOP
        timeB += ((double)(clock()-now)/1000000.0);
...
void CRnnLM::copyHiddenLayerToInput(struct neuron *neu0, struct neuron *neu1)
{
    int a;
...
void* CRnnLM::callThread(void *a)
{
    struct arg_struct *args = (struct arg_struct *) a;
    int id = (int) args->id;
    ((CRnnLM*)args->model)->trainModelThread(id);
    return NULL;
}


void CRnnLM::trainModelThread(int id)
{
    int word=-1, last_word=0, a, b;
    int word_count=0, last_word_count=0;
    FILE *fi;
    clock_t now;

    // initialise neurons
    struct neuron *neu0=NULL, *neu1=NULL, *neuc=NULL, *neu2=NULL;
    int *bptt_history=NULL;
    neuron *bptt_hidden=NULL;
    struct synapse *bptt_syn0=NULL;
    int history[MAX_NGRAM_ORDER];
    initNeurons(&neu0, &neu1, &neuc, &neu2, &bptt_history, &bptt_hidden, &bptt_syn0);


    if (bptt>0) for (a=0; a<bptt+bptt_block; a++) bptt_history[a]=0;
    for (a=0; a<MAX_NGRAM_ORDER; a++) history[a]=0;
    netFlush(neu0, neu1, neuc, neu2);
    ...
    while (1) {
        if (word_count - last_word_count > 10000) {
            ...
            if (debug_mode > 1) {
                now=clock();
                fprintf(stderr, "%c  Alpha: %f  Progress: %.2f%%  Words/thread/sec: %.2fk timeTotal: %.1f\t", 13, alpha, word_count_actual / (real)(train_words + 1) * 100, word_count_actual / ((real)(now - start + 1) / (real)CLOCKS_PER_SEC * 1000), ((double)(now-timeTotal)/1000000.0));
                //fprintf(stderr, "%c  Alpha: %f  Progress: %.2f%%  Words/thread/sec: %.2fk timeTotal: %.1f timeA: %.1f timeB: %.1f\t", 13, alpha, word_count_actual / (real)(train_words + 1) * 100, word_count_actual / ((real)(now - start + 1) / (real)CLOCKS_PER_SEC * 1000), ((double)(now-timeTotal)/1000000.0), timeA, timeB);
                fflush(stdout);
            }
        }

        // break when done with the thread's text
        if (feof(fi) || ((word_count > train_words / num_threads) && (word==0))) break;
    
        word=readWordIndex(fi);     //read next word
        ++word_count;

        computeNet(last_word, word, neu0, neu1, neuc, neu2, history); //compute prob distribution
        ...
        //
        learnNet(last_word, word, neu0, neu1, neuc, neu2, bptt_history, bptt_hidden, bptt_syn0, history, word_count);
        ...
        if (independent && (word==0)) netReset(neu0, neu1, bptt_history, bptt_hidden, history);
    }
    fclose(fi);
...
void CRnnLM::trainNet()
{
    ...
    // create threads to process the documents
    pthread_t *pt = (pthread_t *)malloc(num_threads * sizeof(pthread_t));

    // initialise neurons for valid
    struct neuron *neu0v=NULL, *neu1v=NULL, *neucv=NULL, *neu2v=NULL;
    int *bptt_historyv=NULL;
    neuron *bptt_hiddenv=NULL;
    struct synapse *bptt_syn0v=NULL;
    int historyv[MAX_NGRAM_ORDER];
    initNeurons(&neu0v, &neu1v, &neucv, &neu2v, &bptt_historyv, &bptt_hiddenv, &bptt_syn0v);
    for (a=0; a<MAX_NGRAM_ORDER; a++) historyv[a]=0;
    ...
    while (1) {
        ...
        printf("Iteration %d:\n", iter);
        //one thread for each partition of text
        for (a = 0; a < num_threads; a++) {
            struct arg_struct *args = (struct arg_struct *) malloc(sizeof(arg_struct));
            args->model = (void*) this;
            args->id = a;
            pthread_create(&pt[a], NULL, callThread, (void *)args);
        }
        ...
        //VALIDATION PHASE
        netFlush(neu0v, neu1v, neucv, neu2v);
        ...
        while (1) {
            word=readWordIndex(fi);     //read next word
            computeNet(last_word, word, neu0v, neu1v, neucv, neu2v, historyv); //compute prob dist
            if (feof(fi)) break;        //end of file: report LOGP, PPL
            
    	    if (word!=-1) {
                logp+=log10(neu2v[vocab[word].class_index+vocab_size].ac * neu2v[word].ac);
                wordcn++;
    	    }

            copyHiddenLayerToInput(neu0v, neu1v);

            if (last_word!=-1) neu0v[last_word].ac=0;  //delete previous activation

            last_word=word;
            
            for (a=MAX_NGRAM_ORDER-1; a>0; a--) historyv[a]=historyv[a-1];
            historyv[0]=last_word;

            if (independent && (word==0)) netReset(neu0v, neu1v, bptt_historyv, bptt_hiddenv, historyv);
        }
        fclose(fi);
        ...
        printf("\tlogp = %.8f\n", logp);
        printf("\tllogp = %.8f\n", llogp);
        printf("\tlogp*min_improvement = %.8f\n", logp*min_improvement);
        printf("\talpha_divide = %d\n", alpha_divide);*/
...
void CRnnLM::testNet()
{
    ...
    struct neuron *neu0=NULL, *neu1=NULL, *neuc=NULL, *neu2=NULL;
    struct neuron *neu0b=NULL, *neu1b=NULL, *neucb=NULL, *neu2b=NULL, *neu1b2=NULL;
    int *bptt_history=NULL;
    neuron *bptt_hidden=NULL;
    struct synapse *bptt_syn0=NULL;
    int history[MAX_NGRAM_ORDER];
    ...
    restoreNet();
    initNeurons(&neu0, &neu1, &neuc, &neu2, &bptt_history, &bptt_hidden, &bptt_syn0);
    initBackupNeurons(&neu0b, &neu1b, &neucb, &neu2b, &neu1b2);
    ...
    if (bptt>0) for (a=0; a<bptt+bptt_block; a++) bptt_history[a]=0;
    for (a=0; a<MAX_NGRAM_ORDER; a++) history[a]=0;
    if (independent) netReset(neu0, neu1, bptt_history, bptt_hidden, history);
    
    while (1) {
        
        word=readWordIndex(fi);		//read next word
        computeNet(last_word, word, neu0, neu1, neuc, neu2, history);		//compute probability distribution
        if (feof(fi)) break;		//end of file: report LOGP, PPL
	...
	if (debug_mode>1) {
        ...
        if (dynamic>0) {
            ...
            //
            alpha=dynamic;
    	    learnNet(last_word, word, neu0, neu1, neuc, neu2, bptt_history, bptt_hidden, bptt_syn0, history, wordcn);    //dynamic update
    	}
        copyHiddenLayerToInput(neu0, neu1);
...
{
    int a, word, last_word, wordcn, sentcn;
    FILE *fi, *flog, *lmprob=NULL;
    float prob_other; //has to be float so that %f works in fscanf
    real log_other, log_combine, log_unigram, senp;
    //int nbest=-1;
    int nbest_cn=0;
    ...
    computeNet(0, 0, neu0, neu1, neuc, neu2, history);
    copyHiddenLayerToInput(neu0, neu1);
    saveContext(neu1, neu1b);
    saveContext2(neu1, neu1b2);
    ...
    sentcn=0;
    senp=0;
    std::vector<double> wsenp;
    strcpy(ut1, (char *)"");
    while (1) {
        if (last_word==0) {
            fscanf(fi, "%s", ut2);

            if (nbest_cn==1) saveContext2(neu1, neu1b2);		//save context after processing first sentence in nbest

            
            if (strcmp(ut1, ut2)) {
            strcpy(ut1, ut2);
            nbest_cn=0;
            restoreContext2(neu1, neu1b2);
            saveContext(neu1, neu1b);
            } else restoreContext(neu1, neu1b);
        ...
        /*
        readWord(wordstr, fi);
        word=searchVocab(wordstr);*/
	
        word=readWordIndex(fi);     //read next word
        if (lambda>0) computeNet(last_word, word, neu0, neu1, neuc, neu2, history); //compute probability distribution
        if (feof(fi)) break;        //end of file: report LOGP, PPL
        ...
        if (word!=-1) {
    		...
    		wsenp.push_back(log10(neu2[word].ac*lambda + prob_other*(1-lambda)) / log10((double)vocab[word].cn/train_words) * -1.0);
            log_unigram+=log10((double)vocab[word].cn/train_words);
            wordcn++;
            sentcn++;
        } else {
    	    ...
    	    if (prob_other!=0) {
                ...
                wsenp.push_back(log10(prob_other));
    	    } else {
                logp+=oov_penalty;
                log_other+=oov_penalty;
                log_combine+=oov_penalty;
                senp+=oov_penalty;
                wsenp.push_back(oov_penalty);
    	    }
    	    wordcn++;
            sentcn++;
        }
        ...
        if (word==0) {		//write last sentence log probability / likelihood
    	    fprintf(flog, "%f ", senp);
    	    fprintf(flog, "%d ", sentcn);
    	    fprintf(flog, "%f ", log_unigram);
            int wsenpi;
            for (wsenpi=0; wsenpi<wsenp.size(); wsenpi++) {
                fprintf(flog, "%f", wsenp.at(wsenpi));
                if (wsenpi != wsenp.size()-1) {
                    fprintf(flog, ",");
                } else {
                    fprintf(flog, "\n");
                }
...
void CRnnLM::testGen()
{
    ...
    copyHiddenLayerToInput(neu0, neu1);
    while (wordcn<gen) {
        computeNet(last_word, 0, neu0, neu1, neuc, neu2, history);		//compute probability distribution
