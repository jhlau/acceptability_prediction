...
int main(int argc, char **argv)
{
	...
	printf("\t-old-classes\n");
    	...
    	printf("\t-threads <int>\n");
    	printf("\t\tSet number of threads (default 1)\n");
    	...
    	printf("\t-print-word-class <file>\n");
        printf("\t\tPrint the class index of words in the vocabulary\n");
    ...
    if (i>0) {
        if (i+1==argc) {
            printf("ERROR: threads not specified!\n");
            return 0;
        }
    ...
    if (debug_mode>0) {
        if (independent==0) {
            printf("Sentences will *NOT* be processed independently...\n");
        } else {
    ...
    //print word class?
    i=argPos((char *)"-print-word-class", argc, argv);
    if (i>0) {
        if (i+1==argc) {
            printf("ERROR: word class file not specified!\n");
            return 0;
        }

        strcpy(wordclass_file, argv[i+1]);

        if (debug_mode>0)
        printf("word class file: %s\n", wordclass_file);

        f=fopen(wordclass_file, "rb");
    ...
    if (train_mode) {
        ...
        if (print_word_class) {
            model1.setWordClassFile(wordclass_file);
        }
	...
	model1.setOldClasses(old_classes);
        ...
        model1.setNumThreads(num_threads);
