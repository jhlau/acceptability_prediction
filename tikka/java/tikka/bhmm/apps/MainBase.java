...
public class MainBase {
    ...
    public static Options setOptions() {
        ...
        options.addOption("xd", "pos-dict", true,
              "full path of the pos tag dictionary (semi-supervised learning)");
        options.addOption("xt", "trigram", false,
              "use trigram for HMM (default = bigram)");
        options.addOption("xthread", "num-thread", true,
              "number of threads for parallelisation (default=1)");
        options.addOption("xc", "num-chunks", true,
              "number of chunks (used by Two Tier HMM only) (default=1)");
        options.addOption("xp", "p-hash", true,
              "p hash parameter used by Bayesian Chunker only (default=0.5))");
        return options;
    }
}
