...
/**
 * Newman et al Bayesian Segmenter model
 *
 * @author JH Lau
 */
public class ParNewmanSegmenter extends HMM {
    ...
    public ParNewmanSegmenter (CommandLineOptions options) {
        super(options);
        beta = -1.0;
        delta = -1.0;
        gamma = -1.0;
        ...
        for (int iter = 0; iter < itermax; ++iter) {
            ...
            for (Future<ArrayList<Object>> future : list) {
                try {
                    ...
                    future.get().get(0); //dummy results for now; TODO: change to use no return call
                } catch (Exception e) {
    ...
    private class Worker implements Callable<ArrayList<Object>> {
        ...
        private void computeSentenceProb(ArrayList<Object> sentenceProbResult) {
            ...
            for (int i=0; i<wordVector.length; i++) {
                ...
                if (segid == 1) {
                   logProb += Math.log10(calcChunkProb(c, wChunkFreq, wChunkTokens, wordCounts));
                }
            ...
            for (int i = start; i < end; i++) {
                ...
                if (segid == 1) {
                    ...
                    if (chunksentid != nextchunksentid) {
                        ...
                        h0 = calcChunkProb(newchunk, wChunkFreq, wChunkTokens, wordCounts);
                        // see it as two units llx, rr
                        h1 = calcChunkProb(c, wChunkFreq, wChunkTokens, wordCounts) *
                            calcChunkProb(nextChunk, wChunkFreq, wChunkTokens, wordCounts, 1);
                    }
                ...
                // the case where there hasn't been a boundary before  llxrr | ....
                    ...
                    //see it as one unit llxrr
                    h0 = calcChunkProb(c, wChunkFreq, wChunkTokens, wordCounts);
                    //see it as two units llx, rr
                    String lchunk="", rchunk="";
                    lchunk = flatten(Arrays.copyOfRange(wordVector, segStart, i+1), "+");
                    rchunk = flatten(Arrays.copyOfRange(wordVector, i+1, segEnd+1), "+");

                    h1 = calcChunkProb(lchunk, wChunkFreq, wChunkTokens, wordCounts) *
                        calcChunkProb(rchunk, wChunkFreq, wChunkTokens, wordCounts, 1);
