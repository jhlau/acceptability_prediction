...
Real EvaluateLM(NNet* nnet, const std::string& filename, bool print_logprobs, bool accurate_nce) {
  ...
  SentenceReader reader(nnet->vocab, filename, nnet->cfg.reverse_sentence, kAutoInsertUnk, nnet->cfg.char_lm);
  Real logprob_sum = 0;
  uint64_t n_words = 0;
  ...
  while (reader.Read()) {
    ...
    if (print_logprobs) {
      printf("%f ", -sen_logprob);
      printf("%d ", seq_length);
      
      double log_unigram = 0.0;
      for (int i = 0; i < seq_length; ++i) {
        log_unigram += nnet->vocab.GetWordLogProb(sen[i]);
      }
      printf("%f ", log_unigram);

      for (int i = 0; i < seq_length; ++i) {
        printf("%f", logprob_per_pos[i] / nnet->vocab.GetWordLogProb(sen[i]) * -1.0);
        if (i != (seq_length-1)) {
            printf(",");
        } else {
            printf("\n");
...
void *RunThread(void *ptr) {
  ...
  SentenceReader reader(nnet->vocab, *task.train_file, nnet->cfg.reverse_sentence, kAutoInsertUnk, nnet->cfg.char_lm);
  reader.SetChunk(task.chunk_id, task.total_chunks, nnet->cfg.char_lm);
...
void TrainLM(
  ...
  for (int n_bad_epochs = 0, epoch = 1; n_bad_epochs <= max_bad_epochs; ++epoch) {
    ...
    /*
    std::string tmpy = "NAN";
    entropy = strtod(tmpy.c_str(), NULL);
    entropy = entropy;
    std::cerr << "\nentropy isnan? =" << std::isnan(entropy) << "\n";
    std::cerr << "entropy <0.0? =" << (entropy <0.0) << "\n";
    std::cerr << "entropy >=0.0? =" << (entropy >=0.0) << "\n";
    ...
    if (isnan(entropy) || isinf(entropy) || !(ratio >= bad_ratio) || (-entropy>=0.0)) {
      // !(ratio >= bad_ratio) will catch nan and inf even in fastmath mode
      if (isnan(entropy) || isinf(entropy) || !(ratio >= awful_ratio) || (-entropy>=0.0)) {
...
void SampleFromLM(NNet* nnet, int seed, int n_samples, Real generate_temperature, bool char_lm) {
    ...
    for (std::string line; std::getline(std::cin, line);) {
        ...
        if (char_lm) {
            for (unsigned i=0; i<line.length(); ++i)
            {
                ...
                WordIndex wid = nnet->vocab.GetIndexByWord(token.c_str());
                if (wid == Vocabulary::kWordOOV) {
                    wid = nnet->vocab.GetIndexByWord("X");
                    if (wid == Vocabulary::kWordOOV) {
                    fprintf(stderr, "ERROR Word '%s' is not found in vocabulary;"
                        " moreover, X is not found as well\n", token.c_str());
                    exit(1);
                    }
                }
                wids.push_back(wid);
        ...
        for (size_t i = 0; i < wids.size(); ++i) {
            if (char_lm) {
                if (std::string(nnet->vocab.GetWordByIndex(wids[i])).compare("<sp>")==0) {
                    printf(" ");
                } else {
            ...
            for (; sen.back() != 0;) {
...

                {
                    ...
                    while (1) {
                        ...
                        tmpw = nnet->vocab.GetWordByIndex(order[wid]);
                        if (tmpw.substr(0,3).compare("UNK") != 0) {
                            sen.back() = order[wid];
                ...
                if (char_lm) {
                    if (std::string(nnet->vocab.GetWordByIndex(sen.back())).compare("<sp>")==0) {
                        printf(" ");
                    } else {
                ...
                int max_sent_size = 60;
                if (char_lm) max_sent_size = 600;
                if (sen.size() >= max_sent_size) {
                    break;
                }
...
int main(int argc, char **argv) {
  ...
  opts.Add("char-lm", "Train a character language model (default = 0/false; trains word language model)", &char_lm);
  ...
  if (has_vocab) {
    ...
    vocab.BuildFromCorpus(train_file, show_progress, char_lm);
  ...
  if (n_samples > 0) {
    SampleFromLM(main_nnet, random_seed, n_samples, generate_temperature, main_nnet->cfg.char_lm);
