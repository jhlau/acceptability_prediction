...
const char kSpaceTag[] = "<sp>";
const int kEOSSize = sizeof(kEOSTag);
const int kSpaceSize = sizeof(kSpaceTag);
...
bool WordReader::ReadWord(char* word, bool char_lm) {
    if (char_lm) {
        return ReadChar(word);
    }
    else {
...
bool WordReader::ReadChar(char* word) {
  if (pointer_ == NULL) {
    // loop until EOF or not-empty line
    for (; pointer_ == NULL || *pointer_ == 0;) {
      if (chunk_end_ != -1 && chunk_end_ < ftell(file_)) {
  ...
  if (*pointer_ == 0) {
    ...
    if (IsSpace(*pointer_)) {
        strncpy(word, kSpaceTag, MAX_STRING - 1);
        word[std::min(MAX_STRING, kSpaceSize - 1)] = 0;
    } else {
        word[0] = *pointer_;
        word[1] = 0;
    }
  ...
  //std::cout << "Read a character = " << word << "\n";
...
void WordReader::SetChunk(int chunk, int chunk_count, bool char_lm) {
  ...
  if (chunk != 0) {
    ...
    for (char tmp[MAX_STRING]; ReadWord(tmp, char_lm) && strcmp(tmp, kEOSTag) != 0; ) {}
  }
}
...
double Vocabulary::GetWordLogProb(WordIndex index) const {
    /*
    printf("\n\nword index = %d\n", index);
    printf("word = %s\n", words_[index].word);
    printf("word freq = %d\n", ((int)words_[index].freq));
    printf("vocab size = %d\n", ((int)totalTokens));
    printf("log word prob = %f", (log10(words_[index].freq) - log10(totalTokens)));*/
    return (log10(words_[index].freq) - log10(totalTokens));
}
...
void Vocabulary::BuildFromCorpus(const std::string& fpath, bool show_progress, bool char_lm) {
  char buffer[MAX_STRING];
  AddWord(kEOSTag);

  WordReader reader(fpath.c_str());
  for (int64_t read_words = 0; reader.ReadWord(buffer, char_lm); ++read_words) {
...
SentenceReader::SentenceReader(
        const Vocabulary& vocab, const std::string& filename, bool reverse, bool auto_insert_unk, bool char_lm)
    : WordReader(filename)
    , sentence_length_(0)
    , sentence_id_(-1)
    , vocab_(vocab)
