...
class Vocabulary {
 public:
  ...
  // Returns log probability of a word by its index
  double GetWordLogProb(WordIndex index) const;
  ...
  void BuildFromCorpus(const std::string& fpath, bool show_progress, bool char_lm);
 ...
 private:
  ...
  int64_t totalTokens; //total number of word tokens (i.e. sum of all word frequencies)
...
class WordReader {
 public:
  ...
  // Reads a word/character, depending on char_lm
  bool ReadWord(char* word, bool char_lm);
  // Reads a word; returns false iff EOF
  // Words longer than MAX_STRING are truncated
  bool ReadWord(char* word);
  // Reads a character instead of a word
  bool ReadChar(char* word);

  // Forces the reader to iterate over only a 1 / chunk_count part of the training file
  void SetChunk(int chunk, int chunk_count, bool char_lm);
...
class SentenceReader : public WordReader {
 public:
  SentenceReader(const Vocabulary& vocab, const std::string& filename, bool reverse, bool auto_insert_unk, bool char_lm);
