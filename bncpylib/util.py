...
def files_contents_equality(file1,file2):
    """Checks if the contents of two (text) files are identical. Files are rewinded."""
    file1.seek(0)
    file2.seek(0)
    for line1 in file1:
        try:
...
def collect_words(corpus):
    """Returns the set of words in the corpus"""
    voc = set()
    for line in corpus:
