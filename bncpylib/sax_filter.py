...
# on the basis of properties of their XML content. Instead of using a dom parser we use a sax parser which should be much much faster
...
def parse(expr):
    ...
    return eval(expr,__sax_filter_parse_globals,__sax_filter_parse_locals)

# Base class

class SaxFilter(object):
    """Generic class that actually doesn't filter anything"""

    def accept(self,xml_file):
        """This function should be redefined by subclasses to actually perform tests on the xml file

           It should return True in case the xml file satisfies the constraints encoded by the class and False otherwise.
        """
        return True
...
# Logical combinators
# These defaults are dumb, they should be avoided (a part the negation filter)

class ConjunctionFilter(SaxFilter):
    ...
    def accept(self,xml_file):
        xml_file.seek(0)
        r1 = self.filter1.accept(xml_file)
        xml_file.seek(0)
        r2 = self.filter2.accept(xml_file)
        return r1 and r2


class DisjunctionFilter(SaxFilter):
...
class NegationFilter(SaxFilter):
    ...
    def accept(self,xml_file):
        xml_file.seek(0)
        return not self.filter.accept(xml_file)


# Atomic filters

class TextClassificationFilter(SaxFilter):
    ...
    def accept_wtext(self,xml_file):
        class WTextHandler(xml.sax.handler.ContentHandler):
            def __init__(self,code):
                self.result = False
                self.code = code
            def startElement(self,name,attrs):
                if name == 'wtext' and self.code == attrs.getValue('type'):
                    self.result = True
        h = WTextHandler(self.code)
        self.sax_parser.setContentHandler(h)
        self.sax_parser.parse(xml_file)
        return h.result

    def accept_stext(self,xml_file):
        class STextHandler(xml.sax.handler.ContentHandler):
            def __init__(self,code):
                self.result = False
                self.code = code
            def startElement(self,name,attrs):
                if name == 'stext' and self.code == attrs.getValue('type'):
                    self.result = True
        h = STextHandler(self.code)
        self.sax_parser.setContentHandler(h)
        self.sax_parser.parse(xml_file)
        return h.result

    def accept_tei(self,xml_file):
        class CatRefHandler(xml.sax.handler.ContentHandler):
            def __init__(self,code):
                self.result = False
                self.code = code
            def startElement(self,name,attrs):
                if name == 'catRef':
                    if self.code in attrs.get('targets').split():
                        self.result = True
        h = CatRefHandler(self.code)
        self.sax_parser.setContentHandler(h)
        self.sax_parser.parse(xml_file)
        return h.result

class ElementPresenceFilter(SaxFilter):
    ...
    def accept(self,xml_file):
        class ElementPresenceHandler(xml.sax.handler.ContentHandler):
            ...
            def startElement(self,name,attrs):
                if name == self.element:
                    self.result = True
        h = ElementPresenceHandler(self.elementName)
        self.sax_parser.setContentHandler(h)
        self.sax_parser.parse(xml_file)
        return h.result
...
__sax_filter_parse_globals = { '__builtins__' : None }

__sax_filter_parse_locals = {
